...
static irqreturn_t goodix_ts_threadirq_func(int irq, void *data)
{
	...
	if (atomic_read(&core_data->is_suspend_ing)) {
		ts_info("the irq is in suspending , cencel");
		return IRQ_HANDLED;
	}
...
static int goodix_ts_suspend(struct goodix_ts_core *core_data)
{
	...
	atomic_set(&core_data->is_suspend_ing, 1);
...
out:
	...
	atomic_set(&core_data->is_suspend_ing, 0);
	ts_info("Suspend end");
	return 0;
}
